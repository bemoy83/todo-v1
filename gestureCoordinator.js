...
element.addEventListener('pointerdown', (e) => {
  if (dragHandler.onStart(element, { x: e.clientX, y: e.clientY })) {
	// Gesture started successfully
...
function onUp(e) {
  if (dragHandler.isActive()) {
	if (success) {
	  dragHandler.onComplete();
	} else {
	  dragHandler.onCancel('failed');
	}
  }
}

// SWIPE EXAMPLE:
const swipeHandler = createGestureHandler(gestureCoordinator, 'swipe-task');

// Register cleanup
const unregisterCleanup = gestureCoordinator.onCleanup(() => {
  // Clean up DOM, remove listeners, etc.
